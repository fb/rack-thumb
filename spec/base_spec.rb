...
describe Rack::Thumb do
  ...
  it "should render a thumbnail with a signature" do
    verify_response(request(credentials).get("/media/imagick_50x100-sw-#{signature}.jpg"), "image/jpeg", [50,100])
  end
  ...
  it "should pass non-thumbnail image requests to the application" do
    res = request(credentials).get("/media/imagick.jpg")
  ...
  it "should render a thumbnail with a signature and retina multiplier" do
    verify_response(request(credentials).get("/media/imagick_50x100-sw-#{signature}@4x.jpg"), "image/jpeg", [200, 400])
  end
end
